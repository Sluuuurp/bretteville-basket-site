        ...
        .map((match, index) => {
          ...
          const bgclass =
            index % 2 === 0
              ...
              : 'md:bg-white'
          return `
      <div class="match mt-5 mb-5  flex items-center ${bgclass} font-manrope max-w-375">
        <div class="grid grid-cols-6 font-bold items-center relative">
          ...
          <p class='uppercase text-sm  md:text-center opacity-50 col-start-5 md:col-start-2 xl:col-start-2 '>${match.lieu}</p>
          ...
          <div class='flex h-10 md:h-15 text-center col-start-1 col-span-6 md: md:col-start-3 mt-2'>
            <p class='uppercase w-30 text-[0.65rem] sm:text-xs m-auto'>Bretteville Basket Cingal</p>
            <img class='' src='/img/logo-bbc.png'>
            <div class='flex items-center h-15 md:h-20 gap-3 md:gap-6 font-teko text-2xl md:text-4xl'>
              <span> ${match.scoreBBC || '0'}</span>
              <img class='w-5 md:w-10' src='/img/versus.png'>
              <span>${match.scoreExt}</span>
            </div>
            <img class='ml-10' src=${match.img && !match.img.includes('defaultLogo') ? match.img : '/img/logo-ffbb.jpg'} />
            <p class='uppercase w-30 text-[0.65rem] sm:text-xs  m-auto'>${match.equipe}</p>
          </div>
        </div>
      </div>
    `
